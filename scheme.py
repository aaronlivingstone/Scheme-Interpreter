...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        for elem in range(len(procedure.formals)):
            env.bindings[procedure.formals[elem]] = args[elem]
...
def apply_primitive(procedure, args, env):
    ...
    try:
        if procedure.use_env:
            p_list.append(env)
        return procedure.fn(*p_list)
    except TypeError:
        raise SchemeError("Scheme Error raised.")
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("improper number of formals and values")
...
def do_lambda_form(vals, env):
    ...
    return LambdaProcedure(formals, Pair('begin', rest), env)
...
def do_mu_form(vals):
    ...
    return MuProcedure(formals, Pair('begin', rest))
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        "*** YOUR CODE HERE ***"
        if scheme_symbolp(target[0]) == False:
...
def do_let_form(vals, env):
    ...
    for elem in bindings:
        if scheme_symbolp(elem[0]) and len(elem) <= 2:
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            ...
            for elem in vals:
                if elem[0] == 'else':
                    return Pair('begin', elem.second)                   
                elif scheme_true(scheme_eval(elem[0], env)):
                    if len(elem) <= 1: 
                        return scheme_eval(elem[0], env)
                    return Pair('begin', elem.second)
    return None
...
def check_formals(formals):
    ...
    if not scheme_listp(formals):
            raise SchemeError("malformed list: {0}".format(str(formals)))
    s = set()
    for i in range(len(formals)):
        if not scheme_symbolp(formals[i]):
            raise SchemeError("not symbols")           
        s.add(formals[i])
    if len(s) < len(formals):
        raise SchemeError("formals repeated")
